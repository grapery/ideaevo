...
/**
 * AuthOAuthButtons —— 微信 + Google 登录按钮组。
 * login / signup 共用，保证两个入口的 OAuth 顺序与样式一致。
 */
export function AuthOAuthButtons({
  ...
  onWeChat: () => void;
  onGoogle: () => void;
  wechatLabelKey: "auth.wechatScanLogin" | "auth.wechatScanRegister";
}) {
  const { t } = useI18n();
  return (
    <>
      <button type="button" onClick={onWeChat} className="w-full btn-outline mb-3">
        <span className="text-[#07C160] font-semibold">{t("auth.wechat")}</span>
        {t(wechatLabelKey)}
      </button>
      <button type="button" onClick={onGoogle} className="w-full btn-outline">
