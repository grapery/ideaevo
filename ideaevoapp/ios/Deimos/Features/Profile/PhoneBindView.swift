...
/// S13 绑定手机号 — 画板表单页(账号与安全入口,换绑/补绑流程)。
/// 与 WeChatPhoneBindView 同一版式;此变体走 change_phone 用途并回写当前会话用户。
struct PhoneBindView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 22) {
            AtlasSubPageNavBar(title: "绑定手机号", onBack: { dismiss() })
            ...
            Text("为了保障账号安全，需要绑定手机号。绑定后可用于接收想法互动通知。")
                .font(.system(size: 13))
    ...
    private func verify() async {
        ...
        do {
            let user = try await APIClient.shared.verifyPhoneAsUser(
